...
def generate_index_html(latest_content):
    print("🍏 Building NIUS Responsive Inter UI...")
    ...
    for story in raw_stories:
        ...
        # Smart Title Parser (Preserves Emojis)
...
<head>
    ...
    <style>
        ...
        .nav-center {{ position: absolute; left: 50%; transform: translateX(-50%); white-space: nowrap; }}
        .logo-sub {{ font-weight: 900; font-size: 14px; opacity: 0.4; letter-spacing: -0.02em; }}
        ...
        #theme-toggle {{ cursor: pointer; width: 42px; height: 42px; border-radius: 50%; border: none; background: var(--text); color: var(--bg); display: flex; align-items: center; justify-content: center; }}
        .theme-icon {{ width: 18px; height: 18px; }}
        body.dark .sun, body:not(.dark) .moon {{ display: none; }}
        
        .container {{ max-width: 900px; margin: 120px auto 80px; padding: 0 40px; display: grid; gap: 60px; }}
        
        /* G3 Squircle Layout */
        .squircle {{ border-radius: 42px; background: var(--card); border: 1px solid var(--border); padding: 45px; }}
        .news-card h3 {{ font-size: 34px; margin: 0 0 32px; font-weight: 800; line-height: 1.15; letter-spacing: -0.04em; }}
        ...
        @media (max-width: 768px) {{
            ...
            .container {{ margin: 100px auto 40px; padding: 0 16px; gap: 24px; }}
            .squircle {{ border-radius: 28px; padding: 24px; }}
            .news-card h3 {{ font-size: 22px; margin-bottom: 20px; }}
            .news-card li {{ font-size: 15px; margin-bottom: 10px; }}
            .content-section {{ padding-top: 20px; }}
        }}
        
        .archive-grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 12px; margin-top: 40px; }}
        .archive-btn {{ border-radius: 12px; background: var(--card); padding: 14px; text-decoration: none; color: var(--text); font-weight: 600; text-align: center; border: 1px solid var(--border); font-size: 14px; }}
    </style>
</head>
<body>
    <nav class="nav">
        <div class="logo-main"><div class="pulse"></div> N.I.U.S.</div>
        <div class="nav-center"><span class="logo-sub">news.intelligence.ultimate.source.</span></div>
    ...
    <script>
        function toggleTheme() {{
            ...
            localStorage.setItem('nius-responsive-v1', body.classList.contains('dark') ? 'dark' : 'light');
        }}
        window.onload = () => {{
            if (localStorage.getItem('nius-responsive-v1') === 'dark') document.body.classList.add('dark');
        }};
    </script>
</body>
</html>"""
    ...
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(full_html)
    print("✅ Build Complete: NIUS Responsive Terminal.")
